...
function showMessage(element, message, type) {
    ...
    element.className = element.id === "messageBox"
        ? `message-box ${type}`
        : `modal-message ${type}`;
...
function switchMode(mode) {
    ...
    authSubtitle.textContent = signup
        ? "أنشئ حسابك للبدء والاستفادة من خدمات مواجهة."
        : "سجّل دخولك للوصول إلى حسابك ونتائجك.";
...
function getCode(container) {
    return getCodeInputs(container).map(input => input.value).join("");
}
...
signupForm.addEventListener("submit", async (event) => {
    ...
    if (password.length < 6) {
        showMessage(messageBox, "يجب أن تكون كلمة المرور 6 أحرف على الأقل.", "error");
        return;
    }
    ...
    try {
        ...
        showMessage(messageBox, error.message || "حدث خطأ أثناء إنشاء الحساب.", "error");
    } finally {
...
verifyCodeButton.addEventListener("click", async () => {
    ...
    if (code.length !== 6) {
        showMessage(verificationMessage, "يرجى إدخال الرمز المكون من 6 أرقام.", "error");
        return;
    }
...
resendCodeButton.addEventListener("click", async () => {
    ...
    try {
        ...
        showMessage(verificationMessage, "تم إرسال رمز جديد إلى بريدك الإلكتروني.", "success");
...
loginForm.addEventListener("submit", async (event) => {
    ...
    if (!email || !password) {
        showMessage(messageBox, "يرجى إدخال البريد الإلكتروني وكلمة المرور.", "error");
        return;
    }
    ...
    try {
        ...
        // Temporary step: store the token until we connect the final auth flow.
        localStorage.setItem("muwajeh_token", result.token);
        localStorage.setItem("muwajeh_user", JSON.stringify(result.user));
        ...
        showMessage(messageBox, error.message || "حدث خطأ أثناء تسجيل الدخول.", "error");
    } finally {
...
document.getElementById("sendResetCodeButton").addEventListener("click", async () => {
...
document.getElementById("resetPasswordButton").addEventListener("click", async () => {
    ...
    if (code.length !== 6) {
        showMessage(resetMessage, "يرجى إدخال رمز التحقق المكون من 6 أرقام.", "error");
        return;
    }

    if (password.length < 6) {
        showMessage(resetMessage, "يجب أن تكون كلمة المرور 6 أحرف على الأقل.", "error");
        return;
    }
    ...
    try {
        ...
        showMessage(resetMessage, error.message || "تعذر تحديث كلمة المرور.", "error");
    } finally {
